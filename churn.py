...
from sklearn.model_selection import (
    ...
    RandomizedSearchCV, learning_curve, RepeatedStratifiedKFold,
)
...
df = pd.read_csv(r"C:\Users\chara\OneDrive\Desktop\Churn\Training data\train.csv")
...
df["salary_per_age"]        = df["EstimatedSalary"]  / (df["Age"]              + 1)
...
df["credit_x_active"]       = df["CreditScore"]      *  df["IsActiveMember"]
df["balance_x_active"]      = df["Balance"]          *  df["IsActiveMember"]
...
df["senior_customer"]        = (df["Age"] > 45).astype(int)
df["long_tenure"]            = (df["Tenure"] > 5).astype(int)
df["zero_balance_inactive"]  = ((df["Balance"] == 0) & (df["IsActiveMember"] == 0)).astype(int)
df["senior_x_inactive"]      = df["senior_customer"]  * (1 - df["IsActiveMember"])
df["multi_product_active"]   = (df["NumOfProducts"] > 1).astype(int) * df["IsActiveMember"]

ENGINEERED = [
    "balance_per_product", "salary_per_age", "credit_age_ratio",
    "balance_salary_ratio", "tenure_age_ratio", "active_x_products",
    "credit_x_active", "balance_x_active", "balance_x_credit",
    "salary_x_active", "age_x_products", "credit_x_balance",
    "products_x_active", "has_balance", "senior_customer",
    "long_tenure", "zero_balance_inactive", "senior_x_inactive",
    "multi_product_active"
]
...
def evaluate(name, y_true, y_prob, threshold=None):
    ...
    print(f"\n{'='*55}\nMODEL: {name} | Threshold: {threshold:.2f}\n{'='*55}")
    ...
    return {"name":name,"acc":acc,"auc":auc,"ap":ap,
            "prec":prec,"rec":rec,"f1":f1,
            "threshold":threshold,"y_prob":y_prob}
...
print(f"\nCV1 Stratified K-Fold: {[f'{x:.4f}' for x in s]} Mean:{s.mean():.4f} | ⏱️{time.time()-t:.1f}s")

t = time.time()
rskf = RepeatedStratifiedKFold(n_splits=5, n_repeats=3, random_state=42)
rs   = cross_val_score(xgb_pipe, X_train_p, y_train, cv=rskf, scoring="roc_auc")
print(f"CV2 Repeated K-Fold  : Mean:{rs.mean():.4f} Std:{rs.std():.4f} | ⏱️{time.time()-t:.1f}s")
...
print(f"CV3 RandomizedSearch : Best AUC:{rs_search.best_score_:.4f} | ⏱️{time.time()-t:.1f}s")
print(f"   Best Params: {rs_search.best_params_}")

t = time.time()
ts, tr_s, v_s = learning_curve(
    best_xgb_pipe, X_train_p, y_train,
    cv=StratifiedKFold(n_splits=3, shuffle=True, random_state=42),
...
plt.fill_between(ts, tr_s.mean(1)-tr_s.std(1), tr_s.mean(1)+tr_s.std(1), alpha=0.1)
plt.fill_between(ts, v_s.mean(1)-v_s.std(1),   v_s.mean(1)+v_s.std(1),   alpha=0.1)
plt.xlabel("Training Size"); plt.ylabel("AUC")
plt.title("CV4: Learning Curve — XGBoost"); plt.legend()
plt.tight_layout(); plt.savefig("outputs_train/learning_curve_xgb.png"); plt.close()
...
history = ann.fit(
    ...
    callbacks=[
        ...
        callbacks.ModelCheckpoint("outputs_train/best_ann.h5",
...
for i,(m,vm,title) in enumerate([
    ("loss","val_loss","Loss"),("auc","val_auc","AUC"),("accuracy","val_accuracy","Accuracy")
]):
    axes[i].plot(history.history[m], label="Train")
    axes[i].plot(history.history[vm], label="Val")
    axes[i].set_title(title); axes[i].legend()
plt.tight_layout(); plt.savefig("outputs_train/ann_history.png"); plt.close()
...
lgb_model = lgb.LGBMClassifier(
    ...
    random_state=42, objective="binary", verbosity=-1
)
t = time.time()
lgb_model.fit(X_tr_sm, y_tr_sm)
print(f"✅ LightGBM trained | ⏱️{time.time()-t:.1f}s")
...
for name, r in results.items():
    print(f"{name:<12} {r['acc']:>10.4f} {r['auc']:>8.4f} {r['prec']:>8.4f} {r['rec']:>8.4f} {r['f1']:>8.4f}")
...
plt.tight_layout(); plt.savefig("outputs_train/confusion_matrix.png"); plt.close()
...
plt.tight_layout(); plt.savefig("outputs_train/roc_curve.png"); plt.close()

p_arr, r_arr, _ = precision_recall_curve(y_test, best["y_prob"])
plt.figure(figsize=(7,5))
plt.plot(r_arr, p_arr, label=f"AP={best['ap']:.4f}")
plt.xlabel("Recall"); plt.ylabel("Precision")
plt.title(f"Precision-Recall — {best['name']}"); plt.legend()
plt.tight_layout(); plt.savefig("outputs_train/precision_recall.png"); plt.close()
...
output = pd.DataFrame({
    "CUSTOMER_ID":       range(len(p_all)),
    "CHURN_PROBABILITY": p_all,
    "CHURN_LABEL":       (p_all >= best["threshold"]).astype(int),
    "RISK_TIER":         pd.cut(p_all, bins=[0,0.3,0.6,1.0],
                             labels=["LOW","MEDIUM","HIGH"], include_lowest=True),
    "MODEL_VERSION":     f"train_only_{best['name'].lower()}_v1"
})
output.to_csv("outputs_train/churn_predictions.csv", index=False)
...
# 13. SAVE
# ─────────────────────────────────────────────────────────────
ann.save("outputs_train/ann_model.h5")
joblib.dump(best_xgb_pipe,     "outputs_train/best_xgb_pipe.pkl")
joblib.dump(lgb_model,         "outputs_train/lgb_model.pkl")
joblib.dump(imp,               "outputs_train/imputer.pkl")
joblib.dump(scl,               "outputs_train/scaler.pkl")
joblib.dump(best["threshold"], "outputs_train/best_threshold.pkl")
joblib.dump(FEATURES,          "outputs_train/feature_names.pkl")

print("\n" + "="*55)
print(f"✅ ALL DONE! Best model: {best['name']}")
print("   outputs_train/ folder:")
print("   📄 churn_predictions.csv")
print("   📊 confusion_matrix.png | roc_curve.png")
print("   📊 precision_recall.png | ann_history.png")
print("   📊 learning_curve_xgb.png | shap_importance.png")
